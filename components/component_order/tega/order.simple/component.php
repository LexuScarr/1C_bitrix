...
$isValidationEnabled = (($_POST[$arParams["ENABLE_VALIDATION_INPUT_NAME"]] ?? 'Y') == "N");

\Bitrix\Sale\DiscountCouponsManager::init();

// Создаем заказ и устанавливаем тип плательщика
...
// Доставка – стандартное получение доступных служб доставки
...
foreach ($deliveryList as $delivery) {
	if (
		(intval($form["DELIVERY"] ?? 0) == $delivery["ID"]) ||
		(!isset($form["DELIVERY"]) && empty($arResult["DELIVERY"]))
	) {
...
foreach ($paySystemList as $paySystem) {
	if (
		(intval($form["PAY_SYSTEM"] ?? 0) == $paySystem["ID"]) ||
		(!isset($form["PAY_SYSTEM"]) && empty($arResult["PAY_SYSTEM"]))
	) {
...
$arResult["PRICES"]["TOTAL_PRICE_FORMATTED"] = SaleFormatCurrency($arResult["PRICES"]["TOTAL_PRICE"], $currency);
$arResult["PRICES"]["DELIVERY_PRICE"] = $order->getDeliveryPrice();
$arResult["PRICES"]["DELIVERY_PRICE_FORMATTED"] = SaleFormatCurrency($arResult["PRICES"]["DELIVERY_PRICE"], $currency);
$arResult["PRICES"]["PRODUCTS_PRICE"] = $arResult["PRICES"]["TOTAL_PRICE"] - $arResult["PRICES"]["DELIVERY_PRICE"];
$arResult["PRICES"]["PRODUCTS_PRICE_FORMATTED"] = SaleFormatCurrency($arResult["PRICES"]["PRODUCTS_PRICE"], $currency);
...
while ($property = $properties->fetch()) {
	...
	if ($property["ID"] == $arParams["FIO_PROPERTY"]) $FIO_PROPERTY_CODE = $property["CODE"];
	if ($property["ID"] == $arParams["PHONE_PROPERTY"]) $PHONE_PROPERTY_CODE = $property["CODE"];
	if ($property["ID"] == $arParams["EMAIL_PROPERTY"]) $EMAIL_PROPERTY_CODE = $property["CODE"];
	if ($property["ID"] == $arParams["DATE_PROPERTY"]) $DATE_PROPERTY_CODE = $property["CODE"];
}
...
$customDeliveryPrice = floatval($_POST['DELIVERY_CUSTOM_PRICE'] ?? 0);
if ($customDeliveryPrice > 0) {
	$shipment->setField("CUSTOM_PRICE_DELIVERY", "Y");
	$shipment->setField("PRICE_DELIVERY", $customDeliveryPrice);
}
// Обработка формы
$form = $_POST[$arParams["FORM_NAME"]] ?? [];
$isValidationEnabled = (($_POST[$arParams["ENABLE_VALIDATION_INPUT_NAME"]] ?? 'Y') == "N");
...
if (!empty($form)) {
		...
		foreach ($arResult["ORDER_PROPS"] as $key => $arProperty) {
			...
			foreach ($propertyCollection as $property) {
				if ($property->getField('CODE') == $arProperty["CODE"]) {
					if ($arProperty["CODE"] == "LOCATION") {
						// Фиксируем местоположение (Уфа) через скрытое поле
						$property->setValue($arResult["CURRENT_VALUES"]["ORDER_PROPS"]["LOCATION"]);
					} else {
	...
	$arResult["PRICES"]["TOTAL_PRICE"] = $order->getPrice();
	$arResult["PRICES"]["TOTAL_PRICE_FORMATTED"] = SaleFormatCurrency($arResult["PRICES"]["TOTAL_PRICE"], $order->getCurrency());
	$arResult["PRICES"]["PRODUCTS_PRICE"] = $arResult["PRICES"]["TOTAL_PRICE"] - $arResult["PRICES"]["DELIVERY_PRICE"];
	$arResult["PRICES"]["PRODUCTS_PRICE_FORMATTED"] = SaleFormatCurrency($arResult["PRICES"]["PRODUCTS_PRICE"], $order->getCurrency());
	...
	// Валидация
	$arResult["ERRORS"] = [];
	$arResult["ERRORS_FIELDS"] = [];
	if (!empty($arResult["ORDER_PROPS"])) {
		foreach ($arResult["ORDER_PROPS"] as $key => $arProperty) {
			if (empty($arResult["CURRENT_VALUES"]["ORDER_PROPS"][$arProperty["CODE"]]) &&
				in_array($arProperty["ID"], $arParams["REQUIRED_ORDER_PROPS"])
			) {
				$arResult["ERRORS"][] = \Bitrix\Main\Localization\Loc::getMessage("REQUIRED_FIELD") . " \"" . $arProperty["NAME"] . "\"";
	...
	if (empty($arResult["ERRORS"]) && $arResult["HIDE_ERRORS"] != "Y") {
		...
		if (!$savingResult->isSuccess()) {
			$arResult["ERRORS"] = array_merge($arResult["ERRORS"], $savingResult->getErrorMessages());
		} else {
			$arResult["ORDER_ID"] = $order->GetId();
			if (!empty($arParams["EVENT_TYPES"]) && $USER->IsAuthorized() &&
				(!empty($arResult["USER"]["EMAIL"]) || (isset($EMAIL_PROPERTY_CODE) && !empty($arResult["CURRENT_VALUES"]["ORDER_PROPS"][$EMAIL_PROPERTY_CODE])))
			) {
				$mailContactList = "";
				$mailUserName = "";
				$mailBasketList = "";
				if (isset($PHONE_PROPERTY_CODE) && !empty($arResult["CURRENT_VALUES"]["ORDER_PROPS"][$PHONE_PROPERTY_CODE])) {
					$mailContactList .= \Bitrix\Main\Localization\Loc::getMessage("PHONE_PROPERTY") . $arResult["CURRENT_VALUES"]["ORDER_PROPS"][$PHONE_PROPERTY_CODE] . '<br/>';
				}
				if (isset($EMAIL_PROPERTY_CODE) && !empty($arResult["CURRENT_VALUES"]["ORDER_PROPS"][$EMAIL_PROPERTY_CODE])) {
					$mailContactList .= \Bitrix\Main\Localization\Loc::getMessage("EMAIL_PROPERTY") . $arResult["CURRENT_VALUES"]["ORDER_PROPS"][$EMAIL_PROPERTY_CODE] . '<br/>';
				}
				if (isset($DATE_PROPERTY_CODE) && !empty($arResult["CURRENT_VALUES"]["ORDER_PROPS"][$DATE_PROPERTY_CODE])) {
					$mailContactList .= \Bitrix\Main\Localization\Loc::getMessage("DATE_PROPERTY") . $arResult["CURRENT_VALUES"]["ORDER_PROPS"][$DATE_PROPERTY_CODE] . '<br/>';
				}
				if (isset($FIO_PROPERTY_CODE) && !empty($arResult["CURRENT_VALUES"]["ORDER_PROPS"][$FIO_PROPERTY_CODE])) {
				...
				if (!empty($mailBasket)) {
					foreach ($mailBasket as $basketItem) {
						$mailBasketList .= $basketItem->getField('NAME') . ' ' . $basketItem->getQuantity() . ' x ' .
							SaleFormatCurrency($basketItem->getPrice(), $currency) . ' = ' .
							SaleFormatCurrency($basketItem->getFinalPrice(), $currency) . '</br>';
					}
				}
				$arEventFieldsUs = [
					...
					"EMAIL" => (isset($EMAIL_PROPERTY_CODE) && !empty($arResult["CURRENT_VALUES"]["ORDER_PROPS"][$EMAIL_PROPERTY_CODE])) ?
