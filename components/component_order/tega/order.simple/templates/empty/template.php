...
<script src="https://api-maps.yandex.ru/2.1/?apikey="></script>
<script src="<?= $templateFolder ?>/js/streets.js" defer></script> <!-- Загрузка списка улиц -->
<script src="<?= $templateFolder ?>/js/delivery.js"></script> <!-- Логика доставки и карты -->
...
<form method="post"
    ...
    <input type="hidden"
           ...
           value="Y">
    <input type="hidden" id="DELIVERY_CUSTOM_PRICE" name="DELIVERY_CUSTOM_PRICE" value="" />
    ...
    <!-- Свойства заказа (например, имя, адрес, дата) -->
    <?
	    ...
	    foreach ($arResult["ORDER_PROPS"] as $arProp) { ?>
            <label for="<? echo $arParams["FORM_NAME"] ?>_<?= $arProp["CODE"] ?>">
                ...
                <? if (
                    ...
                    <!-- Поле местоположения (зафиксировано "Уфа") -->
                <? } elseif ($arProp["CODE"] == "LOCATION") { ?>
	                <?php $defaultLocationCode = "0000728734"; ?> <!-- Код местоположения для Уфы -->
                    <input type="hidden"
                           name="<? echo $arParams["FORM_NAME"] ?>[<?= $arProp["CODE"] ?>]"
                           value="<? echo $defaultLocationCode; ?>">
                    <span>Уфа</span>

                    <!-- Поле ввода адреса с картой -->
                <? } elseif ($arProp["CODE"] == "ADDRESS") { ?>
                    <input type="text"
                           name="<? echo $arParams["FORM_NAME"] ?>[ADDRESS]"
                           id="address"
                           placeholder="Введите адрес доставки (улица, дом, квартира)"
                           value="<? echo $arResult["CURRENT_VALUES"]["ORDER_PROPS"]["ADDRESS"]; ?>">
                    <div id="suggestions" class="suggestions"></div> <!-- Контейнер для автодополнения адреса -->
                    <h2>Выберите точку на карте для установки адреса доставки</h2>
                    <div id="map" style="width: 100%; height: 400px; margin-top: 20px;"></div> <!-- Карта Яндекса -->
                    <div id="result"></div> <!-- Результат расчета доставки -->
                <? } else { ?>
    ...
    <textarea name="<? echo $arParams["FORM_NAME"] ?>[USER_COMMENT]" id="comment">
        <? echo $arResult["CURRENT_VALUES"]["ORDER_PROPS"]["USER_COMMENT"]; ?>
    </textarea>

    <input type="hidden" name="COUPON" id="COUPON_HIDDEN" value="" />

    <!-- Блок ввода промокода -->
    <div class="coupon-block">
        <label for="COUPON">Промокод:</label>
        <input type="text" name="COUPON" id="COUPON" placeholder="Введите промокод" value="" />
        <button type="button" id="applyCouponBtn">Применить купон</button>
        <div id="couponResult" style="margin-top: 5px; color: green;"></div>
    ...
    <table class="order-price-table">
        <tr>
            <td><? echo GetMessage("ORDER_PRICE"); ?></td>
            <td id="order_price"><? echo $arResult["PRICES"]["PRODUCTS_PRICE_FORMATTED"]; ?></td> <!-- Стоимость товаров -->
        </tr>
        <tr>
            <td><? echo GetMessage("DELIVERY_PRICE"); ?></td>
            <td id="delivery_price"><? echo $arResult["PRICES"]["DELIVERY_PRICE_FORMATTED"]; ?></td> <!-- Стоимость доставки -->
        </tr>
        <tr>
            <td><? echo GetMessage("TOTAL_PRICE"); ?></td>
            <td id="total_price"><? echo $arResult["PRICES"]["TOTAL_PRICE_FORMATTED"]; ?></td> <!-- Общая стоимость -->
        </tr>
    </table>

    <!-- Согласие пользователя (если включено) -->
